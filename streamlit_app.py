    ...
    for idx, t in enumerate(lower_texts):
        for word in search_terms:
            ...
            if start_idx != -1:
                ...
                # Предполагается, что индексы совпадают с оригинальным текстом
...
def process_file_for_processing(file_bytes: bytes, filename: str, col_name: str, dict_brands_models: Dict[str, str], translit_enabled: bool) -> pd.DataFrame:
    ...
    html_list = []
    for txt in series:
        plain, html_preview = process_text(txt, {}, dict_brands_models, translit_enabled)
        plain_list.append(plain)
        html_list.append(html_preview)
    df_result = df.copy()
    # Обновляем только обработанный столбец и его preview
    df_result[col_name] = plain_list
    df_result[f"{col_name}_preview_html"] = html_list
    # Преобразуем только эти два столбца в строки для избежания ошибок сериализации
    df_result[col_name] = df_result[col_name].astype(str)
    df_result[f"{col_name}_preview_html"] = df_result[f"{col_name}_preview_html"].astype(str)
    return df_result
