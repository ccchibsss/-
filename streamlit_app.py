#!/usr/bin/env python3
# Полный исправленный код с улучшенной функцией prepare_additions_fast и интеграцией вывода двух столбцов
...
car_brands_models: Dict[str, str] = {
    ...
    # Mercedes-Benz (уже есть выше, без повторов)
    # Volkswagen (без повторов)
    ...
    try:
        with open(ADDITIONS_FILE, "r", encoding="utf-8") as f:
            ...
            if isinstance(loaded, dict):
                car_brands_models.update({str(k): str(v)
...
LAT_TO_CYR_RULES = [
    ...
    ("j", "й"), ("'", "ь"), ('"', "ъ"),
    ("x", "кс"), ("q", "к"), ("w", "в"),
    # Вариации для мягкости, твердины, диакритики Вариации для мягкости, твердины, диакритики
    ("'", "ь"),
    ('"', "ъ"),
    # Вариации для мягкости, твердины, диакритики Расширенные варианты с учетом различных транслитерационных систем и вариантов написания
    ("shch", "щ"),
    ("sht", "шт"),
    ("sci", "щи"),
    ("sci", "щи"),
...
def format_custom(text: str, final_struct: Dict) -> str:
    ...
    pattern_pairs = re.findall(
        r'\([^\)]+\)|[A-Za-z0-9\-\.]+|[А-Яа-я0-9\-\.]+', text_no_years)
    brand = ""
    model = ""
    if pattern_pairs:
    ...
    if brand:
        ...
        if val:
            ru_brand = val[1] if isinstance(
                val, (list, tuple)) and len(val) > 1 else ""
    if model:
        val = mp.get(model.lower())
        if val:
            ru_model = val[1] if isinstance(
                val, (list, tuple)) and len(val) > 1 else ""
    parts = []
    if brand:
        parts.append(brand)
            ...
            if part in ['/', ';']:
                ...
                if key_lower in final_struct["map"]:
                    ...
                    if translit_allowed and contains_latin(part) and not contains_cyrillic(part):
                        ...
                        result_parts.append(
                            f"{part} ({decline_word_cached(cyr)})")
                    else:
                        result_parts.append(part)
...
def save_additions():
    try:
        with open(ADDITIONS_FILE, "w", encoding="utf-8") as f:
            json.dump({str(k): str(v) for k, v in {**car_brands_models,
                      **added_pairs}.items()}, f, ensure_ascii=False, indent=2)
    except Exception:
        pass
    ...
    for candidate in candidates:
        for base in base_keys:
            ...
            if similarity_full >= threshold:
                ...
                similarity_letters = SequenceMatcher(None, "".join(
                    candidate_letters), "".join(base_letters)).ratio()
...
def run_streamlit_app() -> None:
    ...
    st.markdown(
        "Загрузите CSV/XLSX, выберите столбец — скрипт автоматически подсветит совпадения.")

    sidebar = st.sidebar
    threshold = sidebar.slider(
        "Порог для автодобавления", 0.6, 0.99, 0.85, 0.01)
    translit_allowed = sidebar.checkbox(
        "Автотранслитерация (латиница → кириллица)", value=True)

    sidebar.header("Загрузить словарь (опционально)")
    dict_file = sidebar.file_uploader(
        "Файл словаря (json/csv/xlsx)", type=["json", "csv", "xls", "xlsx"])
    ...
    uploaded = st.file_uploader(
        "Выберите CSV/XLSX", type=["csv", "xls", "xlsx"])
    external_url = sidebar.text_input(
        "URL внешних данных (CSV/XLSX) — необязательно")
    if not uploaded:
    ...
    try:
        if uploaded.name.lower().endswith(('.xls', '.xlsx')):
            ...
            try:
                ...
                txt = raw.decode(CSV_ENCODING, errors="ignore") if isinstance(
    ...
    st.success(
        f"Файл: {uploaded.name} ({df.shape[0]} строк, {df.shape[1]} колонок)")
    st.dataframe(df.head(5))
    col = st.selectbox("Выберите столбец для обработки", df.columns.tolist())

    if st.button("Обработать"):
        ext_df = load_external_data(
            external_url) if external_url else pd.DataFrame()

        series = df[col]
        dataset_words = extract_words_from_series(series)
        external_words = extract_words_from_series(
            ext_df.stack()) if not ext_df.empty else set()
        ...
        additions = prepare_additions_fast(
            base_keys, candidates, threshold=threshold)
        if additions:
        ...
        df["Исходное"] = df[col]
        df["Обработанное"] = df[col].astype(str).apply(lambda v: process_text_fast(
            v, final_struct, translit_allowed=translit_allowed))
        ...
        if export_format == "Excel":
            ...
            csv_bytes = df.to_csv(
                index=False, encoding=CSV_ENCODING).encode(CSV_ENCODING)
...
def main():
    ...
    ext_df = load_external_data(
        args.external) if args.external else pd.DataFrame()
    ...
    # Экспорт
    output_path = args.output or ("result.xlsx" if args.input.lower().endswith(
        ('.xls', '.xlsx')) else "result.csv")
    try:
