...
"RSX": "РСХ",
"TLX": "ТЛКС",
# ... (оставьте весь ваш словарь здесь без изменений)
# для примера я оставил часть, добавьте весь свой словарь сюда
}
...
def detect_language(text: str) -> str:
    # Простая проверка: если в тексте есть латинские буквы, считаем, что язык английский
...
def process_text(text: str, dict_brands_models: dict, translit_enabled: bool) -> str:
    ...
    # Создаем словарь для быстрого поиска по ключам (независимо от регистра)
    dict_map = {k.lower(): v for k, v in dict_brands_models.items()}

    # Разбиваем текст на слова и знаки препинания
    tokens = re.findall(r'\b\w+\b|[^\w\s]', seg, flags=re.UNICODE)
    ...
    for i, token in enumerate(tokens):
        ...
        # Проверка по словарю
        if token_lower in dict_map:
            tokens[i] = dict_map[token_lower]
        elif translit_enabled:
            # Попытка транслитерации
            tk = transliterate(token, 'lat2cyr').lower()
            if tk in dict_map:
                tokens[i] = dict_map[tk]

    # Если язык английский, переводим весь текст в кириллицу
    if lang == 'en':
        text_translit = transliterate(' '.join(tokens), 'lat2cyr')
        return f"{original} - ({text_translit})"

    # Собираем обратно строку
    processed_seg = ''
    for t in tokens:
        if re.match(r'[^\w\s]', t):  # знаки препинания
            processed_seg += t
        else:
            if processed_seg:
                processed_seg += ' '
    ...
    return f"{original} - ({processed_seg})"
